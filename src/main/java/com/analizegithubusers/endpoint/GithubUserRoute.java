package com.analizegithubusers.endpoint;

import com.analizegithubusers.dto.AggregationResultDto;
import com.analizegithubusers.handler.GithubUserHandler;
...
public class GithubUserRoute {
    ...
    @RouterOperations({
            ...
            @RouterOperation(path = "/api/v1/github-users/{githubUsername}", method = RequestMethod.GET,
                    operation = @Operation(operationId = "matchingGithubUsers",
                            parameters = @Parameter(name = "githubUsername", in = ParameterIn.PATH, description = "the github login"), tags = "githubUser", summary = "Search users by username that partially matching",
                            security = @SecurityRequirement(name = "bearerAuth"),
                            responses = @ApiResponse(responseCode = "200", content = @Content(array = @ArraySchema(schema = @Schema(implementation = GithubUserResponseDto.class)))))),
    ...
    @Bean
    public RouterFunction<ServerResponse> githubRouter(GithubUserHandler handler) {
        return RouterFunctions
                .route(GET("/api/v1/github-users").and(accept(APPLICATION_JSON)), handler::getAll)
                .andRoute(GET("/api/v1/github-users/{githubUsername}").and(accept(APPLICATION_JSON)), handler::findByUsernamePartiallyMatching)
